...
struct dynamic_buffer_view::vtables
{
  ...
  constexpr static dynamic_buffer_view::vtable_t dynamic_buffer_vtable_{
      .size     =[](const void *this_, std::size_t) { return static_cast<const Buffer *>(this_)->size(); },
  ...
  constexpr static dynamic_buffer_view::vtable_t contiguous_vtable_{
      .size     = [](const void *this_, std::size_t pos) { return pos; },
      .max_size = [](const void *this_) { return static_cast<const Buffer *>(this_)->max_size(); },
      .capacity = [](const void *this_) { return static_cast<const Buffer *>(this_)->capacity(); },
      ...
      .prepare  =[](void *this_, std::size_t n, std::size_t position)
      {
        ...
        mutable_buffer buf{self->data(), position + n};
        buf += position;
      ...
      {
        position += n;
        static_cast<Buffer *>(this_)->resize(position);
      },
...
dynamic_buffer_view::dynamic_buffer_view(boost::circular_buffer<T, Allocator> &other)
    : this_(&other), vtable_(vtables::circular_vtable_<boost::circular_buffer<T, Allocator>>)
{
}
