...
struct dynamic_buffer_view
{
  ...
  std::size_t          size    () const        { return vtable_.size    (this_); }
  std::size_t          max_size() const        { return (std::min)(vtable_.max_size(this_), max_size_); }
  ...
  dynamic_buffer_view(circular_buffer<T, Allocator> &other);
 ...
 private:
  struct vtable_t
  {
    std::size_t          (*size)    (const void * this_) = 0;
  ...
  std::size_t position_{0u}, max_size_;
};
