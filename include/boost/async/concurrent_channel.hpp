...
#ifndef BOOST_ASYNC_CONCURRENT_CHANNEL_HPP
#define BOOST_ASYNC_CONCURRENT_CHANNEL_HPP
...
/// A concurrent_channel for messages.
/**
 * The basic_concurrent_channel class template is used for sending messages between
 ...
 * messages supported by a concurrent_channel is specified by its @c Traits and
 ...
 *
 * The basic_concurrent_channel class template is not thread-safe, and would typically be
 * used for passing messages between application code that runs on the same
 * thread or in the same strand. Consider using @ref basic_concurrent_concurrent_channel,
 * and its alias template @c experimental::concurrent_concurrent_channel, to pass messages
 * between code running in different threads.
 */
template<typename T = void>
struct concurrent_channel
{

  /// The underlying asio implementation type.
  typedef asio::experimental::concurrent_channel<asio::io_context::executor_type, void(system::error_code, T)> implementation_type;

  /// The type of the executor associated with the concurrent_channel.
  typedef typename asio::io_context::executor_type executor_type;

  /// Construct a basic_concurrent_channel.
  /**
   * This constructor creates and concurrent_channel.
   *
   * @param ex The I/O executor that the concurrent_channel will use, by default, to
   * dispatch handlers for any asynchronous operations performed on the concurrent_channel.
   *
   * @param max_buffer_size The maximum number of messages that may be buffered
   * in the concurrent_channel.
   */
  concurrent_channel(const executor_type& ex, std::size_t max_buffer_size = 0) : impl_(ex, max_buffer_size)
  {
  }

  /// Construct and open a basic_concurrent_channel.
  /**
   * This constructor creates and opens a concurrent_channel.
   *
   * @param context An execution context which provides the I/O executor that
   * the concurrent_channel will use, by default, to dispatch handlers for any asynchronous
   * operations performed on the concurrent_channel.
   *
   * @param max_buffer_size The maximum number of messages that may be buffered
   * in the concurrent_channel.
   */
  concurrent_channel(asio::io_context& context, std::size_t max_buffer_size = 0) : impl_(context, max_buffer_size) {}

  /// Move-construct a basic_concurrent_channel from another.
  /**
   * This constructor moves a concurrent_channel from one object to another.
   *
   * @param other The other basic_concurrent_channel object from which the move will occur.
   *
   * @note Following the move, the moved-from object is in the same state as if
   * constructed using the @c basic_concurrent_channel(const executor_type&) constructor.
   */
  concurrent_channel(concurrent_channel&& other) = default;
  /// Move-assign a basic_concurrent_channel from another.
  /**
   * This assignment operator moves a concurrent_channel from one object to another.
   ...
   * constructor.
   */
  concurrent_channel& operator=(concurrent_channel&& other) = default;

  /// Destructor.
  ~concurrent_channel() = default;
  ...
  /// Get the capacity of the concurrent_channel's buffer.
  std::size_t capacity() noexcept
  {
    return impl_.capacity();
  }

  /// Determine whether the concurrent_channel is open.
  bool is_open() const noexcept
  {
    return impl_.is_open();
  }

  /// Reset the concurrent_channel to its initial state.
  void reset()
  {
    impl_.reset();
  }

  /// Close the concurrent_channel.
  void close()
  {
    impl_.close();
  }

  /// Cancel all asynchronous operations waiting on the concurrent_channel.
  /**
   * All outstanding send operations will complete with the error
   * @c boost::asio::experimental::error::concurrent_channel_cancelled. Outstanding receive
   * operations complete with the result as determined by the concurrent_channel traits.
   */
  void cancel()
  {
    impl_.cancel();
...
template<typename T>
bool concurrent_channel<T>::  try_receive(std::optional<std::tuple<system::error_code, T>> & result)
{
...
template<typename T>
bool concurrent_channel<T>::  try_send(T && value)
{
...
template<>
struct concurrent_channel<void>
{

  /// The underlying asio implementation type.
  typedef asio::experimental::concurrent_channel<asio::io_context::executor_type, void(system::error_code)> implementation_type;
...
#endif //BOOST_ASYNC_CONCURRENT_CHANNEL_HPP
