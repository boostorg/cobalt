      ...
      enable_await_executor<composition_promise<Args...>>
{
  ...
  auto await_transform(asio::deferred_async_operation<void(Args_...), Initiation, InitArgs...> op_)
  {
    struct deferred_op : op<Args_...>
    {
      ...
      op<Args_...>::awaitable_base operator co_await()
      {
        return static_cast<op<Args_...>&&>(*this).operator co_await().replace_resource(resource);
      }
    };
  ...
  {
    return cobalt::as_tuple(static_cast<Op>(op_).operator co_await().replace_resource(handler.get_allocator().resource()));
  }
