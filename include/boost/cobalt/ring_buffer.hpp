...
struct ring_buffer
{
 ...
 private:
  ...
  struct read_op : intrusive::list_base_hook<intrusive::link_mode<intrusive::auto_unlink> >
  {
    ...
    struct cancel_impl;
    bool await_ready() { return !buf->buffer_.empty() || buf->is_closed_; }
  ...
  struct write_op
  {
    ...
    bool await_ready() { return !buf->read_queue_.empty() || buf->is_closed_; }
...
struct ring_buffer<void>
{
 ...
 private:
  ...
  struct read_op : intrusive::list_base_hook<intrusive::link_mode<intrusive::auto_unlink> >
  {
    ...
    struct cancel_impl;
    bool await_ready() { return buf->n_ > 0 || buf->is_closed_; }
    template<typename Promise>
    BOOST_COBALT_MSVC_NOINLINE
    std::coroutine_handle<void> await_suspend(std::coroutine_handle<Promise> h);
