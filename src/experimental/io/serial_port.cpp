...
#include <boost/cobalt/experimental/io/serial_port.hpp>
...
void serial_port::initiate_read_some_(void * this_, mutable_buffer_sequence buffer, boost::cobalt::completion_handler<boost::system::error_code, std::size_t> handler)
{
  initiate_async_read_some(static_cast<serial_port*>(this_)->serial_port_, buffer, std::move(handler));
}
void serial_port::initiate_write_some_(void * this_, const_buffer_sequence buffer, boost::cobalt::completion_handler<boost::system::error_code, std::size_t> handler)
{
