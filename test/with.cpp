...
{
    co_await
            coro::with(finalizer_test{co_await asio::this_coro::executor},
                [](finalizer_test & ft) -> coro::task<void>
                {
                    co_return ;
                });

    finalizer_test f1{co_await asio::this_coro::executor};
    co_await coro::with(f1, [](finalizer_test & ft) -> coro::task<void> { co_return ; });
    ...
    try
    {
        co_await coro::with(f2, [](finalizer_test & ft) -> coro::task<void> { throw std::runtime_error("foobar") ; });
    }
