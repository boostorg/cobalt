...
<2> Default the delay to 100ms, and set it if an argument gets passed in.
<3> Take the executor from the current coroutine promise.
...
is defined as above. This has a few advantages:
 ...
 - the asio::signal_set is connected to cancellations (i.e. `Ctrl+C` causes cancellations)
