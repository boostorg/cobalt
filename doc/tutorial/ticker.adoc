...
for which a generator is a good choice. Note that we'll set it to <<pro_active,pro_active>> so it
eagerly resumes after a `co_yield`.
...
<3> Read a frame from the websocket
<4> Parse & `co_yield` it as an object.
