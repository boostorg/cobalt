...
Note that promise is eager. Calling `echo` will immediately execute code until the `async_read_some`
...
----
<1> Set the generator to `pro_active` mode.
<2> Constructor the listener (destruction will cause cancellation!)
<3> When we have more than 10 workers, we wait for one to finish
<4> Accept a new connection & launch it.
