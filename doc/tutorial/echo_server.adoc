...
Let's write an echo server. We'll be using the `use_op` token everywhere,
so we're using a default completion token:
...
The promise is pretty much the default coroutine; it's an eager coroutine
...
<2> The loop is infinite because it's either canceled server-side or the user closes the socket.
Either way will lead to a logged exception.
<3> Read as much as is available.
<4> Write all the read bytes.

Note that promise is eager. Calling `echo` will immediately execute code until the `async_ready_some`
...
This is a coroutine that can be co_awaited multiple times, while it `co_yield` until `co_return`.
...
----
<1> Cancellation will also lead to an exception here
